...
function renderEmployeeTable(data) {
    ...
    tbody.innerHTML = data.length === 0 ? `<tr><td colspan="5" style="text-align:center;">ไม่พบข้อมูล</td></tr>` : "";
    data.forEach(emp => {
        tbody.innerHTML += `
            <tr>
                ...
                <td style="text-align:center;">
                    <button class="btn approve" style="background-color: #f39c12; margin-right: 5px;" onclick="prepareEditEmployee('${emp.empid}')">แก้ไข</button>
...
function prepareEditEmployee(empid) {
    ...
    document.getElementById('oid').value = emp.oid || ''; // ดึงค่า OID มาแสดง
...
document.getElementById('addEmployeeForm').addEventListener('submit', async (e) => {
    ...
    const payload = {
        ...
        oid: document.getElementById('oid').value, // ส่งค่า OID ไปให้หลังบ้าน
...
function renderCustomerTable(data) {
    ...
    data.forEach(cus => {
        tbody.innerHTML += `
            <tr>
                <td style="text-align:center;">${cus.cid}</td>
                <td>${cus.fname} ${cus.lname}</td>
                <td>${cus.email || '-'}</td>
                <td style="text-align:center;">${cus.tel || '-'}</td>
                <td style="text-align:center;">
                    <button class="btn approve" style="background-color: #f39c12;" onclick="prepareEditCustomer('${cus.cid}')">แก้ไข</button>
