...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // ตรวจสอบทีละตาราง: customer -> employee -> owner
    let found = null;
    let role = null;
 
    // 1. เช็ค customer
    const customerResult = await pool.query(
      `SELECT cid AS id, fname, email FROM customer WHERE email=$1 AND cpassword=$2`,
      [email, password]
    );
    if (customerResult.rows.length > 0) {
      found = customerResult.rows[0];
      role = 'customer';
    }
    ...
    if (!found) {
      const employeeResult = await pool.query(
        `SELECT empid AS id, fname, email FROM employee WHERE email=$1 AND upassword=$2`,
        [email, password]
      );
      if (employeeResult.rows.length > 0) {
        found = employeeResult.rows[0];
        role = 'employee';
      }
    }
    ...
    if (!found) {
      const ownerResult = await pool.query(
        `SELECT oid AS id, fname, email FROM owner WHERE email=$1 AND opassword=$2`,
        [email, password]
      );
      if (ownerResult.rows.length > 0) {
        found = ownerResult.rows[0];
        role = 'owner';
      }
    }
